...
const OAuth = () => {
  ...
  const handleGithubLogin = useCallback(async () => {
    try {
      ...
      // Check if user dismissed or cancelled the OAuth flow
      if (authSessionResult?.type === 'dismiss') {
        console.log('❌ User dismissed the GitHub OAuth popup');
        console.log(
          '💡 Try again: Make sure to complete the GitHub sign-in process'
        );
        return;
      }

      if (authSessionResult?.type === 'cancel') {
        console.log('❌ User cancelled the GitHub OAuth flow');
        return;
      }

      // Check for success type
      if (authSessionResult?.type === 'success') {
        console.log('✅ GitHub OAuth popup completed successfully');
      }
